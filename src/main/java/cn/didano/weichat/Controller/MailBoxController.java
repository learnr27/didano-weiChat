...
public class MailBoxController {
	...
	@PostMapping(value = "parentGetMailId/{own_id}/{type}")
	@ResponseBody
			...
			if (notices.size() != 0) {
				...
				for (int i = 0; i < notices.size(); i++) {
					if (type == 0) {
						...
						if (notices.get(i).getNoticeType() == NoticeType.TeacherChat_Parent.getIndex()) {
							mailId = notices.get(i).getSourceId();
							data = findReply_ByNoticeId(mailId).getData();
							// 设置为已读
							noticeService.setNoticeRead(own_id, notices.get(i).getId());
	 ...
	 */
	@PostMapping(value = "mail_findtByBossId/{own_id}/{user_type}/{type}")
	...
	public Out<OutList<Tb_notice>> notice_findtByUserid(@PathVariable("own_id") Integer own_id,
			@PathVariable("user_type") byte user_type, @PathVariable("type") Integer type) {
